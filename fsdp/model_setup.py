"""
Tokenizer and model initialization utilities for the FSDP trainer.

This module owns the pre-FSDP model setup path so the trainer can consume a
single explicit result object.  It also provides FSDP-specific wrapping and
...
def prepare_training_components(args, device, master_process, logger=None, file_logger=None):
    """Build tokenizer/model state needed by the trainer before FSDP wrapping."""
