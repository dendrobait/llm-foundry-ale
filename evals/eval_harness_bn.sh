#!/bin/bash -l
#############################################
# LM Evaluation Harness - Bengali Language Models
# 
# This script automates the evaluation of Bengali language models using the lm-evaluation-harness framework.
...
#SBATCH --job-name=eval-harness-bengali    # <-- Name of the job (appears in squeue)
...
export MODELS_TO_EVAL="Polygl0t/LilTii-v0.1 Polygl0t/LilTii-v0.2"               # <-- HuggingFace model IDs to evaluate
export EVAL_OUTPUT_DIR="$workdir/.evals_bengali"                                # <-- Directory for evaluation results (YAML files)
export LOGS_FOLDER="$workdir/.eval_logs_bengali"                                # <-- Directory for evaluation logs and JSON results
...
for i in $(seq 0 $((NUM_TO_EVAL - 1))); do
    OUT_FILES+=("$workdir/job_outputs/out-eval-bengali-auto-$((i+1)).$SLURM_JOB_ID")
...
for i in $(seq 0 $((NUM_TO_EVAL - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$gpu_id python3 $workdir/lm_evaluation_harness_bengali/lm_eval \
        --model huggingface \
